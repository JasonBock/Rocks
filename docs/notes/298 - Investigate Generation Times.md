...
* See if profiling brings up anything in Rocks that may be worth changing
* Add tests that have event handlers with type parameters and constraints, along with members that have constraints (e.g. `TService Get<TService>()`)
* Probably want to go back and add:
``` c#
/// <remarks>
