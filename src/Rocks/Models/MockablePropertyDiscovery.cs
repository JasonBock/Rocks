		...
		HashSet<ITypeSymbol> shims, ref uint memberIdentifier)
	{
		...
		if (mockType.TypeKind == TypeKind.Interface)
		{
				...
				.Where(_ => !(_.IsStatic && !_.IsAbstract && !_.IsVirtual) && (_.IsIndexer || _.CanBeReferencedByName)))
			{
				...
				{
					var result = new PropertyMockableResult(selfProperty, mockType,
			...
			foreach (var baseInterfacePropertyGroup in baseInterfacePropertyGroups)
			{
				if (baseInterfacePropertyGroup.Count == 1)
				{
					...
					var result = new PropertyMockableResult(baseInterfacePropertyGroup[0], mockType,
				...
				else
				{
					...
					{
						var result = new PropertyMockableResult(baseInterfaceProperty, mockType,
		...
		this.Properties = new(properties.ToImmutable(), inaccessibleAbstractMembers);
	}
