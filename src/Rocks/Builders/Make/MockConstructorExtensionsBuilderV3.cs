...
internal static class MockConstructorExtensionsBuilderV3
{
	internal static void Build(IndentedTextWriter writer, TypeMockModel mockType)
	{
		...
		if (mockType.Constructors.Length > 0)
		{
			...
			{
				MockConstructorExtensionsBuilderV3.Build(writer, mockType.Type,
		...
		bool requiredInitObjectInitialization, bool hasRequiredProperties)
	{
		...
		var constructorPropertiesParameter =
			requiredInitObjectInitialization ?
				...
				string.Empty;
		var selfParameter =
			$"this global::Rocks.MakeGeneration<{type.FullyQualifiedName}> @{namingContext["self"]}{constructorPropertiesParameter}";
		...
		writer.WriteLine($"internal {(isUnsafe ? "unsafe " : string.Empty)}static {type.FullyQualifiedName} Instance({instanceParameters})");
		writer.WriteLine("{");
		writer.Indent++;
