...
internal static class MockIndexerBuilderV3
{
		...
		uint memberIdentifier, bool raiseEvents, string signature)
	{
		...
		MockMethodVoidBuilderV3.BuildMethodHandler(writer, method, indexer.MockType, namingContext, raiseEvents);
		...
		Models.PropertyModel indexer, bool raiseEvents)
	{
		...
		var signature = MockIndexerBuilderV3.GetSignature(indexer.Parameters, false);
		...
		var indexerSignature = $"{explicitTypeName}{MockIndexerBuilderV3.GetSignature(indexer.Parameters, true)}";
		...
		if (isGetterVisible)
		{
			MockIndexerBuilderV3.BuildGetter(writer, indexer,
		...
		if (isSetterVisible)
		{
			MockIndexerBuilderV3.BuildSetter(writer, indexer, 
				visibility, memberIdentifier, raiseEvents, signature);
		}
