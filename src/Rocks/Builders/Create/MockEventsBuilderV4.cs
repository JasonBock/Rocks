...
internal static class MockEventsBuilderV4
{
	...
	internal static void Build(IndentedTextWriter writer, ImmutableArray<EventModel> events)
	{
		...
		foreach (var @event in events)
		{
			...
			{
				MockEventsBuilderV4.BuildImplementation(writer, @event);
			}
			else
			{
				MockEventsBuilderV4.BuildExplicitImplementation(writer, @event);
			}
		}
