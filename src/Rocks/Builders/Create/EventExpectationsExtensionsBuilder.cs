...
internal static class EventExpectationsExtensionsBuilder
{
	internal static void Build(IndentedTextWriter writer, TypeMockModel mockType)
	{
		if (mockType.Events.Length > 0)
		{
			...
			{
				writer.WriteLine();
				EventExpectationsExtensionsBuilder.BuildAdornments(writer, mockType, "Method");
			}
			...
			if (mockType.Properties.Length > 0)
			{
				...
				{
					writer.WriteLine();
					EventExpectationsExtensionsBuilder.BuildAdornments(writer, mockType, "Property");
				}
				if (mockType.Properties.Any(_ => _.IsIndexer))
				{
					writer.WriteLine();
					EventExpectationsExtensionsBuilder.BuildAdornments(writer, mockType, "Indexer");
				}
			}
		}
	}
