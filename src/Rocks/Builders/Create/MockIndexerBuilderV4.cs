...
internal static class MockIndexerBuilderV4
{
		...
		uint memberIdentifier, bool raiseEvents, string signature)
	{
		...
		writer.WriteLines(
			...
			{{visibility}}get
			{
				if (this.expectations.handler{{memberIdentifier}}.Count > 0)
				{
					foreach (var @{{namingContext["handler"]}} in this.expectations.handler{{memberIdentifier}})
					{
			""");
		...
		uint memberIdentifier, bool raiseEvents, string signature)
	{
		...
		writer.WriteLine("{");
		writer.Indent++;
		MockMethodVoidBuilderV4.BuildMethodHandler(writer, method, indexer.MockType, namingContext, raiseEvents);
		...
		PropertyModel indexer, bool raiseEvents)
	{
		...
		var signature = MockIndexerBuilderV4.GetSignature(indexer.Parameters, includeOptionalParameterValues);
		...
		if (isGetterVisible)
		{
			MockIndexerBuilderV4.BuildGetter(writer, indexer,
		...
		if (isSetterVisible)
		{
			MockIndexerBuilderV4.BuildSetter(writer, indexer, 
				visibility, memberIdentifier, raiseEvents, signature);
		}
