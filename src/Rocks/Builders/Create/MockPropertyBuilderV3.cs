...
internal static class MockPropertyBuilderV3
{
		...
		uint memberIdentifier, bool raiseEvents, string explicitTypeName)
	{
		...
		var handlerName = property.Type.IsPointer ?
			MockProjectedTypesAdornmentsBuilderV3.GetProjectedHandlerInformationFullyQualifiedNameName(property.Type, property.MockType) :
		...
		uint memberIdentifier, bool raiseEvents, string explicitTypeName, bool allowNull)
	{
		...
		var argType = property.Type.IsPointer ?
			PointerArgTypeBuilderV3.GetProjectedFullyQualifiedName(property.Type, property.MockType) :
				property.Type.IsRefLikeType ?
					RefLikeArgTypeBuilderV3.GetProjectedFullyQualifiedName(property.Type, property.MockType) :
		...
		Models.PropertyModel property, bool raiseEvents)
	{
		...
		if (isGetterVisible)
		{
			MockPropertyBuilderV3.BuildGetter(writer, property, visibility, memberIdentifier, raiseEvents, explicitTypeName);
			memberIdentifier++;
		}

		if (isSetterVisible)
		{
			MockPropertyBuilderV3.BuildSetter(writer, property, visibility, memberIdentifier, raiseEvents, explicitTypeName, property.AllowNull);
		}
